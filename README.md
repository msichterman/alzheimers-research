...
Wrangler's browser login **cannot grant DNS write** — `dns_records:write` isn't in its OAuth scope list — so attaching a custom domain over pre-existing records needs an **API token** (Create Token → Custom token; Account → Workers Scripts:Edit, Zone → Workers Routes:Edit, Zone → DNS:Edit, scoped to the `alzheimers.dev` zone). Put it in `.env.cloudflare` as `CLOUDFLARE_API_TOKEN`. User and account-owned tokens both work — they just verify at different endpoints, so an account-owned token reads as invalid against `/user/tokens/verify` while working perfectly. A token set there outranks the login session, so a wrong value breaks every `pnpm cf` command until you comment it out.
